...
def test_i18n() -> None:
    ...
    assert speakhuman.naturaltime(three_seconds) == "3 seconds ago"
    assert speakhuman.ordinal(5) == "5th"
    assert speakhuman.precisedelta(one_min_three_seconds) == "1 minute and 7 seconds"

    try:
        speakhuman.i18n.activate("ru_RU")
        assert speakhuman.naturaltime(three_seconds) == "3 секунды назад"
        assert speakhuman.ordinal(5) == "5ый"
        assert speakhuman.precisedelta(one_min_three_seconds) == "1 минута и 7 секунд"
...
def test_intcomma() -> None:
    ...
    assert speakhuman.intcomma(number) == "10,000,000"

    try:
        speakhuman.i18n.activate("de_DE")
        assert speakhuman.intcomma(number) == "10.000.000"
        assert speakhuman.intcomma(1_234_567.8901) == "1.234.567,8901"
        assert speakhuman.intcomma(1_234_567.89) == "1.234.567,89"
        assert speakhuman.intcomma("1234567,89") == "1.234.567,89"
        assert speakhuman.intcomma("1.234.567,89") == "1.234.567,89"
        assert speakhuman.intcomma("1.234.567,8") == "1.234.567,8"

        speakhuman.i18n.activate("fr_FR")
        assert speakhuman.intcomma(number) == "10 000 000"
        assert speakhuman.intcomma(1_234_567.89) == "1 234 567.89"
        assert speakhuman.intcomma("1 234 567.89") == "1 234 567.89"

        speakhuman.i18n.activate("pt_BR")
        assert speakhuman.intcomma(number) == "10.000.000"
...
def test_naturaldelta() -> None:
    ...
    assert speakhuman.naturaldelta(seconds) == "1,234 years"

    try:
        speakhuman.i18n.activate("fr_FR")
        assert speakhuman.naturaldelta(seconds) == "1 234 ans"
        speakhuman.i18n.activate("es_ES")
        assert speakhuman.naturaldelta(seconds) == "1,234 años"
...
def test_intword_i18n(locale: str, number: int, expected_result: str) -> None:
    try:
        speakhuman.i18n.activate(locale)
    except FileNotFoundError:
        pytest.skip("Generate .mo with scripts/generate-translation-binaries.sh")
    else:
        assert speakhuman.intword(number) == expected_result
    finally:
        speakhuman.i18n.deactivate()
...
def test_langauge_codes(locale: str, expected_result: str) -> None:
    try:
        ...
        assert speakhuman.ordinal(5) == expected_result
    finally:
        speakhuman.i18n.deactivate()
...
def test_ordinal_genders(
    ...
    try:
        ...
        assert speakhuman.ordinal(number, gender=gender) == expected_result
    finally:
        speakhuman.i18n.deactivate()


def test_default_locale_path_defined__spec__() -> None:
    i18n = importlib.import_module("speakhuman.i18n")
...
class TestActivate:
    expected_msg = (
        "Speakhuman cannot determinate the default location of the"
    ...
    def test_en_locale(self) -> None:
        three_seconds = NOW - dt.timedelta(seconds=3)
        test_str = speakhuman.naturaltime(three_seconds)

        speakhuman.i18n.activate("en_US")
        assert test_str == speakhuman.naturaltime(three_seconds)

        speakhuman.i18n.activate("en_GB")
        assert test_str == speakhuman.naturaltime(three_seconds)
    ...
    def test_none_locale(self) -> None:
        ...
        try:
            speakhuman.i18n.activate("fr")
            assert speakhuman.naturaltime(three_seconds) == "il y a 3 secondes"

            speakhuman.i18n.activate(None)
            test_str = speakhuman.naturaltime(three_seconds)

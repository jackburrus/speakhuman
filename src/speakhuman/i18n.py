...
def activate(
    ...
    Raises:
        FileNotFoundError: If speakhuman cannot find the locale folder.
    """
    ...
    if path is None:
        msg = (
            "Speakhuman cannot determinate the default location of the 'locale' folder. "
            "You need to pass the path explicitly."
        )
        raise FileNotFoundError(msg)
    if locale not in _TRANSLATIONS:
        translation = gettext_module.translation("speakhuman", path, [locale])
